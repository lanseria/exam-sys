...
<div class="wrap">
	<div class="main">
		...
		<form class="form-group" method="post" action="<?php echo U('Home/Index/p');?>">
			<div class="det_text">
				<?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$mygroups): $mod = ($i % 2 );++$i;?><p class="para"><?php echo ($mygroups["gname"]); ?>。<?php echo ($mygroups["gdescription"]); ?></p>
					<?php if(is_array($mygroups["question"])): $i = 0; $__LIST__ = $mygroups["question"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$resvo): $mod = ($i % 2 );++$i; switch($resvo["qtype"]): case "T": ?><p class="para rccount"><?php echo ($resvo["qorder"]); ?>、<?php echo ($resvo["question"]); ?></p>
								<div class="row">
									<div class="col-sm-12">
										<?php if(is_array($resvo["answer"])): $i = 0; $__LIST__ = $resvo["answer"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$anssub): $mod = ($i % 2 );++$i;?><div class="radio">
												<input type="radio" name="T<?php echo ($resvo["qorder"]); ?>" id="r<?php echo ($anssub['aorder']); echo ($resvo["qorder"]); ?>" value="<?php echo ($anssub['aorder']); ?>" />
												<label for="r<?php echo ($anssub['aorder']); echo ($resvo["qorder"]); ?>">
													<span><?php echo ($anssub['aorder']); ?>、</span>
													<?php echo ($anssub['answer']); ?>
												</label>
											</div><?php endforeach; endif; else: echo "" ;endif; ?>
									</div>
								</div><?php break;?>
							<?php case "D": ?><p class="para rccount"><?php echo ($resvo["qorder"]); ?>、<?php echo ($resvo["question"]); ?></p>
								<div class="row">
									<div class="col-sm-12">
										<?php if(is_array($resvo[answer])): $i = 0; $__LIST__ = $resvo[answer];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$anssub): $mod = ($i % 2 );++$i;?><div class="checkbox checkbox-circle">
												<input type="checkbox" class="styled" name="D<?php echo ($resvo["qorder"]); ?>[]" id="r<?php echo ($anssub['aorder']); echo ($resvo["qorder"]); ?>" value="<?php echo ($anssub['aorder']); ?>"/>
												<label for="r<?php echo ($anssub['aorder']); echo ($resvo["qorder"]); ?>">
													<span><?php echo ($anssub['aorder']); ?>、</span>
													<?php echo ($anssub['answer']); ?>
												</label>
							...
							<?php case "K": ?><p class="para"><?php echo ($resvo["qorder"]); ?>、<?php echo ($resvo["question"]); ?></p><?php break;?>
							<?php case "J": ?><p class="para"><?php echo ($resvo["qorder"]); ?>、<?php echo ($resvo["question"]); ?></p>
								<textarea class="form-control autosize-transition" name="J<?php echo ($resvo["qorder"]); ?>" id="answer-3-<?php echo ($resvo["qorder"]); ?>" placeholder="解答："></textarea><?php break;?>
							<?php default: endswitch; endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
			</div>
			<div class="clear"></div>
